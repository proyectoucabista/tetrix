      ...
      repeat
                    ...
                    for i:=1 to length(cadena) do
                      if (cadena[i]=',') OR (cadena[i]='(') OR (cadena[i]=')') OR (cadena[i]=':') OR (cadena[i]=';') OR (cadena[i]='<') OR (cadena[i]='>') OR (cadena[i]='/') then
                        ...
                        if salir=false then
                          writeln('Por favor introduzca Un correo Valido!!');
...
procedure Dibujar(x,y:integer);                 // Procedimiento Dibujar (x, y): dibuja una parte de una figura, una parte de una valla de vidrio en una determinada coordenada de la pantalla de texto
begin
  gotoxy(x*2+20,23-y);                      // Colocar el cursor de texto en la línea (X * 2 + 20) por X y (23-Y) la posición inicial del vaso
  if Recorrer=0 then
  begin
   write('. ');                 // Si Recorrer = 0, se muestran dos símbolos con un fondo vacío entre comillas '. 'Para borrar los rastros del movimiento de la figura a través de la pantalla.

  end;
  if Recorrer=1 then
  begin
   write('[]');                 // Recorrer = 1 entonces se emite el bloque de figuras de Tetris '[]'
  end;
  if Recorrer=pared then  // Recorrer = 9 Los caracteres de ASCII # 124  paredes de Tetris '||'
  begin
    textcolor(LightCyan);
  ...
  if (Recorrer=3) and (MatrizPrincipal[x,y]>0) then // si en donde hace la figura hay alguna cantidad entonces se para el juego
   Tope_Matriz:=1;    // Recorrer = 3 y el índice de matriz no es un espacio vacío (el borde de vidrio), luego asigne pus = 1
...
end;  // Fin del procedimiento para dibujar celdas Dibujar (x, y)
// procediemiento para dibujar formas
procedure Figura(x,y,n,s:integer); // El procedimiento figura (x, y, n, s) almacena información sobre los tipos de figuras de Tetris
begin
  if s=3 then
  Tope_Matriz:=0;
   ...
   for x:=1 to 11 do
    for y:=1 to 11 do
      begin
        ...
        Dibujar(x,y);    // Llamada al procedimiento Dibujar (x, y) - dibuja una parte de una figura, una parte de una valla de vidrio en una determinada coordenada de la pantalla de texto
      end;
